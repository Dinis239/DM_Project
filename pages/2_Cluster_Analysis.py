...
def load_cluster_data(classified_path, centroids_path):
    ...
    if not os.path.exists(classified_path) or \
       not os.path.exists(centroids_path):
    ...
    # Standardize 'Final_Cluster' name
    for df_data in [df_classified, df_centroids]:
        ...
        if cluster_col:
            df_data.rename(columns={cluster_col: 'Final_Cluster'}, inplace=True)
        else:
...
df_classified, df_centroids = load_cluster_data(
    CLASSIFIED_FILE, CENTROIDS_FILE
)
...
# --- Session State for Description Toggle ---
...
st.markdown("Use the controls below to select the clusters you want to "
            "review in detail.")
...
for col in df_centroids_display.columns:
    ...
    is_reason_or_time = ('reason:' in col.lower() or 
    ...
    if (is_feature or is_reason_or_time) and is_numeric:
        # Enforce two decimal places and string formatting (.2f)
...
# --- SECTION 1: Cluster Count Distribution (Plotly) ---
...
# --- SECTION 2: Detailed Centroids Table ---
...
# --- SECTION 3: Cluster Descriptions (Conditional) ---

# Button moved here, directly preceding the content it controls
st.button(
    f"{'Hide' if st.session_state.show_descriptions else 'Show'} Descriptions", 
...
if st.session_state.show_descriptions:
    ...
    if description_col_name in df_centroids_display.columns:
        ...
        for i, row in df_centroids_display.iterrows():
            ...
            name_for_title = (
                cluster_name if cluster_name else f"Cluster {cluster_id}"
            )
...
# --- SECTION 4: Visual Cluster Comparison (Radar Chart) ---

st.header("Visual Cluster Comparison (Radar Chart)")
st.markdown("Compare the **normalized** profiles of selected clusters across "
            "multiple dimensions.")
...
with col_features:
    interpretable_display_names = [
        CENTROID_COLUMN_MAP[col] for col in INTERPRETABLE_FEATURES 
...
if radar_selected_clusters and radar_selected_features:
    ...
    df_melted = df_normalized.melt(
        id_vars=['Final_Cluster'], value_vars=radar_selected_features, 
        var_name='Feature', value_name='Normalized_Value'
    )
    df_melted['Feature'] = df_melted['Feature'].map(CENTROID_COLUMN_MAP)

    fig_radar = px.line_polar(
        df_melted, r='Normalized_Value', theta='Feature', 
        color='Final_Cluster', line_close=True,
...
# --- SECTION 5: Cluster Scatter Plot (Plotly) ---
...
if not df_classified.empty:
    ...
    numerical_for_plot = [
        col for col in df_classified.columns if col in INTERPRETABLE_FEATURES
    ]

    if len(numerical_for_plot) >= 2:
        col_x, col_y = st.columns(2)

        # Determine sensible defaults for the axes
        default_x_idx = (
        ...
        if default_x_idx == default_y_idx and len(numerical_for_plot) > 1:
             default_y_idx = 1
        elif default_x_idx == default_y_idx and len(numerical_for_plot) == 1:
             default_y_idx = 0
        ...
        fig_scatter = px.scatter(
            ...
            y=scatter_y,
            color='Final_Cluster',
            title=f'Clustering by {CENTROID_COLUMN_MAP.get(scatter_x)} '
        ...
        # Adjust legend position to top right, outside the plot area
        ...
        st.warning("Not enough numerical columns available in classified data "
                   "for scatter plot.")
else:
